...
//下面采用TIM8的CH2通道输出互补波形【PC7和PB0】


#include "pwm.h"
#include "lcd.h"
/////////////////////////////////////////////在 system_stm32f4xx.c的315行及后面修改了系统分频系数，改变了系统时钟
...
//定时器TIM8初始化
	...
	TIM_BDTRInitStruct.TIM_DeadTime=dead; //死区寄存器存放的数值为28 
	...
	TIM_Cmd(TIM8,ENABLE); //定时器1使能
			...
			//LCD_ShowString(0,380,480,24,24,"                                                        ");//相当于局部清屏
			LCD_ShowNum(0,380,period,8,24);
			LCD_ShowNum(120,380,prescaler,10,24);
			LCD_ShowNum(270,380,pulse,10,24);
			
			//LCD_ShowString(0,420,480,24,24,"                                                        ");//相当于局部清屏
			LCD_ShowNum(270,420,dead,10,24);//显示计算后写入死区的值（0---65535）
			LCD_ShowNum(200,420,sign,10,24);//死区标志参考
}
...
void TIM8_PWM_Init()
{
	...
	GPIO_InitStruct.GPIO_Mode=GPIO_Mode_AF;	//复用推挽输出模式
	GPIO_InitStruct.GPIO_Pin=GPIO_Pin_0;	//互补通道引脚为PB0
	GPIO_InitStruct.GPIO_Speed=GPIO_Speed_50MHz;
	...
	//定时器TIM1初始化
	TIM_TimeBaseInitStruct.TIM_ClockDivision= TIM_CKD_DIV1;	//时钟分割0x0000
	TIM_TimeBaseInitStruct.TIM_CounterMode=TIM_CounterMode_Up;	//向上计数
	//TIM_TimeBaseInitStruct.TIM_CounterMode=TIM_CounterMode_CenterAligned1;	//【也是向上计数？？？？？？？？？？？？？？？？？】
	TIM_TimeBaseInitStruct.TIM_Period=41999;	//自动重装的初值【周期】【65535时，对应最小频率】
	TIM_TimeBaseInitStruct.TIM_Prescaler=3;	//【预分频值】【分频系数】
	//TIM_TimeBaseInitStruct.TIM_RepetitionCounter=0;	//重复计数设置为0
	TIM_TimeBaseInit(TIM8,&TIM_TimeBaseInitStruct);
	...
	TIM_OCInitStruct.TIM_Pulse=21000;		//用来调节输出脉冲的占空比(1/4)
	TIM_OC1Init(TIM8,&TIM_OCInitStruct);	//		【选择CH1】
	TIM_OC1PreloadConfig(TIM8,TIM_OCPreload_Enable);  //比较寄存器（TIM1->CCR1）重装载值使能 
	
	TIM_OCInitStruct.TIM_Pulse=21000;		//用来调节输出脉冲的占空比(1/2)
	TIM_OC2Init(TIM8,&TIM_OCInitStruct);	//		【选择CH2】
	TIM_OC2PreloadConfig(TIM8,TIM_OCPreload_Enable);
