...
//                  SPFD5408/1505/B505/C505/NT35310/NT35510/SSD1963等		  		【4.3寸大屏幕5510】  
...
{ 	
  ...
  GPIO_InitTypeDef  GPIO_InitStructure;	//【GPIO结构体】
	FSMC_NORSRAMInitTypeDef  FSMC_NORSRAMInitStructure;	//【FSMC结构体】
  FSMC_NORSRAMTimingInitTypeDef  readWriteTiming; 	//【读时序结构体】
	FSMC_NORSRAMTimingInitTypeDef  writeTiming;		//【写时序结构体】
  ...
  FSMC_NORSRAMInitStructure.FSMC_Bank = FSMC_Bank1_NORSRAM4;//  这里我们使用NE4 ，也就对应BTCR[6],[7]。【根据硬件不同进行不同的配置】
...
//【根据不同的ID进行不同的初始化】
	...
	LCD_Display_Dir(0);		//默认为竖屏	//【设置LCD显示方向】
	LCD_LED=1;				//点亮背光
	LCD_Clear(BLUE);		//【清屏，让屏幕变成白色】
}  
//清屏函数
//color:要清屏的填充色
...
//num:要显示的字符:" "--->"~"
//size:字体大小 12/16/24
//mode:叠加方式(1)还是非叠加方式(0)	【叠加方式会保留底色，非叠加方式会改变底色】
...
{  							  
	...
	{   
		...
		else if(size==32)temp=asc2_3232[num][t];	//调用2412字体
		else return;								//没有的字库
		
		for(t1=0;t1<8;t1++)
		{			    
			if(temp&0x80)LCD_Fast_DrawPoint(x,y,POINT_COLOR);	//【POINT_COLOR为全局变量，主函数中有赋值】
...
//显示字符串
//x,y:起点坐标
//width,height:区域大小【字符串显示区域的宽度和高度，起点坐标为该区域的左上角坐标】  
//size:字体大小
//*p:字符串起始地址		  
...
{         
    ...
    while((*p<='~')&&(*p>=' '))//判断是不是非法字符!
    {       
        if(x>=width)		//【横坐标超过宽度显示，则下一行对齐显示】
				{
					x=x0;
					y+=size;
				}
        if(y>=height)		//【纵坐标超出高度显示范围，退出不再显示】		// 【   y/size+1表示显示的行数，余数部分y%size时可能会与下一行重叠，故最好满足 height>=size * 显示行数】
					break;//退出
				
        LCD_ShowChar(x,y,*p,size,0);
        x+=size/2;	//【横坐标右移半个字符大小】
        p++;	//【字符指针后移，显示下一个字符】
    }  
}
...
{
	u8 temp;//存储在字库中查找到的当前汉字编码
	u8 t1,t;
	u16 y0=y;
	u8 csize=(size/8+((size%8)?1:0))*size;//得到字体一个字符对应点阵集所占的字节数	【高和宽相等】
	//num=num-'频';
	
	for(t=0;t<csize;t++)//【按字节循环】
	{  
		//下面开始判断字库
		if(size==16);
			//temp=asc2_1616[num][t];	//调用1608字体
		else if(size==24)
...
void LCD_ShowChinese(u16 x,u16 y,u16 width,u16 height,u8 size,u8 *p)
{
	...
	width+=x;
	height+=y;
    while((*p<=13)&&(*p>=0))//判断是不是非法字符!				【若果要添加，如何判断】
    {       
        if(x>=width)		//	【水平超出长度限制，换行显示】
				{
					x=x0;
					y+=size;
				}
        if(y>=height)		//【竖直超出高度限制，多余的不显示】
					break;//退出
				
        LCD_ShowChar(x,y,*p,size,0);//显示当前单个字符
        x+=size;
        p++;
    } 
}
