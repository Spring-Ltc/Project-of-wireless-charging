...
void TIM8_PWM_Init()
{
	...
	TIM_OCInitStruct.TIM_Pulse=2;		//用来调节输出脉冲的占空比(1/4)
	TIM_OC1Init(TIM8,&TIM_OCInitStruct);	//		【选择CH1】
	TIM_OC1PreloadConfig(TIM8,TIM_OCPreload_Enable);  //比较寄存器（TIM1->CCR1）重装载值使能 
	
	TIM_OCInitStruct.TIM_Pulse=2;		//用来调节输出脉冲的占空比(1/2)
	TIM_OC2Init(TIM8,&TIM_OCInitStruct);	//		【选择CH2】
	TIM_OC2PreloadConfig(TIM8,TIM_OCPreload_Enable);
...
void Calculate_Regester(u32 Freq,u32 Duty,u32 Dead,u8 choose)
{
	...
	{
		dead=((Dead/2)/5.95)+128+0-64;
	}
	else if(Dead<=3000)//情况三
	{
		dead=((Dead/8)/5.95)+128+64+0-32;
	}
	else if(Dead<=6000)//情况四
	{
		dead=Dead/16/5.95-32+128+64+32;
	}
